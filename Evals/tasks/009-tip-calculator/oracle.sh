#!/usr/bin/env bash
# Oracle for 009-tip-calculator. Pass if: builds, has TextField, Slider/Stepper, total computation.
set -u
WORK="${1:?usage: oracle.sh <workdir>}"
cd "$WORK" || { echo "no workdir"; exit 2; }
[[ -f Package.swift ]] || { echo "FAIL: no Package.swift"; exit 1; }
if ! swift build 2> .build-stderr.log; then echo "FAIL: swift build error"; tail -20 .build-stderr.log; exit 1; fi

src="$(find . -name '*.swift' -not -path './.build/*' -exec cat {} +)"
check() { if ! echo "$src" | grep -qE "$2"; then echo "FAIL: missing $1"; exit 1; fi; }

check 'TextField for bill'                'TextField'
check 'Slider or Stepper for tip %'       'Slider\(|Stepper\('
check 'multiplication for tip'            '\*\s*(tip|percent|rate|0\.|1\.)|(tip|percent|rate)\s*\*'
check 'addition for total'                '\+\s*(tip|amount|bill)|(bill|amount|subtotal)\s*\+'

echo "PASS"
exit 0
